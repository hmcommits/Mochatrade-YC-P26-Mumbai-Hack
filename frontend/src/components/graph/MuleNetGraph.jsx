...
export function MuleNetGraph({ onInvestigateAccount }) {
  ...
  const handleInvestigateNow = () => {
    ...
    // Find matching case
    let matchedCase = currentDataset.cases?.find(c => c.cluster === target.cluster) || currentDataset.cases?.[0];
    
    // If no case was officially generated by the backend yet, build an ad-hoc one so the button always works
    if (!matchedCase) {
        matchedCase = {
            id: `CASE-MANUAL-${target.label.replace('ACC_','')}`,
            cluster: target.cluster,
            risk: target.riskScore || Math.round(Math.random() * 20 + 70),
            status: 'INVESTIGATING',
            accounts: [target.label],
            tx: [],
            summary: `Analyst-initiated manual investigation for ${target.type} node ${target.label}. Topology indicates elevated structural risk.`,
            dwellRows: []
        };
    }
